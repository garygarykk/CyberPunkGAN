...
from GAN.vgg import Vgg19
from GAN.image_processing import gram, rgb_to_yuv
...
class NeonGanLoss:
    ...
    def compute_loss_G(self, fake_img, img, fake_logit, neon_gray):
        '''
        ...
        anime_feat = self.vgg19(neon_gray)
    ...
    def compute_loss_D(self, fake_img_d, real_neon_d, real_neon_gray_d, real_neon_smooth_gray_d):
        return self.wadvd * (
            self.adv_loss_d_real(real_neon_d) +
            self.adv_loss_d_fake(fake_img_d) +
            self.adv_loss_d_fake(real_neon_gray_d) +
            0.2 * self.adv_loss_d_fake(real_neon_smooth_gray_d)
        )
